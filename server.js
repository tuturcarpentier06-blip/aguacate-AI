...
// Si une clé OpenRouter existe, on utilise OpenRouter.
// Sinon, si une clé OpenAI existe, on utilise directement OpenAI.
const USING_OPENROUTER = Boolean(OPENROUTER_API_KEY) || (!OPENAI_API_KEY && process.env.AI_BASE_URL !== 'https://api.openai.com/v1');
const AI_API_KEY = OPENROUTER_API_KEY || OPENAI_API_KEY;
const AI_BASE_URL = process.env.AI_BASE_URL || (USING_OPENROUTER ? 'https://openrouter.ai/api/v1' : 'https://api.openai.com/v1');
const AI_MODEL = process.env.AI_MODEL || (USING_OPENROUTER ? 'openrouter/auto' : 'gpt-5.5');
const MAX_FILE_MB = Math.max(1, Math.min(15, Number(process.env.MAX_FILE_MB || 10)));
const ECO_MAX_LITRES = Number(process.env.ECOGUACATE_MAX_LITRES || 24.8);
...
const openai = AI_API_KEY ? new OpenAI({
  ...
  defaultHeaders: USING_OPENROUTER ? {
    'HTTP-Referer': process.env.APP_URL || 'https://aguacate-ai.onrender.com',
    'X-Title': 'Aguacate AI v4.0.0'
  } : undefined
}) : null;
...
app.get('/health', (req, res) => res.json({
  ...
  version: '4.0.0',
  ai: !!openai,
  provider: USING_OPENROUTER ? 'openrouter' : 'openai',
  model: AI_MODEL,
  baseURL: AI_BASE_URL
}));
...
async function askAI(messages) {
  if (!openai) {
    throw new Error('Aucune clé IA configurée. Ajoute OPENROUTER_API_KEY (OpenRouter) ou OPENAI_API_KEY (OpenAI) dans Render.');
  }
  ...
  if (typeof reply !== 'string' || !reply.trim()) {
    throw new Error(`Réponse IA vide (modèle: ${AI_MODEL})`);
  }
...
app.post('/chat', chatLimiter, ensureAuth, async (req,res) => {
  try {
    ...
    if(moderationHit(u,message))return res.status(403).json({ok:false,error:'auto-banned',bannedUntil:u.bannedUntil,message:'Ton message contient un langage interdit. Ton accès a été suspendu pendant 24 heures.'});
    ...
    res.status(502).json({
      ok:false,
      error:'ai-error',
      message:'🥑 Le service IA ne répond pas correctement pour le moment. Vérifie la clé API, le modèle et la configuration IA dans Render.'
    });
  }
});
